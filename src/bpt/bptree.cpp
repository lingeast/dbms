...
bp_tree::bp_tree(const bp_tree& that) : fName(that.fName.c_str()), file(NULL), fhelp(NULL), key_itr(NULL) {
...
bp_tree::~bp_tree() {
	//std::cout << "Calling bp_tree destructor" << std::endl;
	if (file != NULL) fclose(file);
	if (fhelp != NULL) delete fhelp;
	if (key_itr != NULL) delete key_itr;
	//std::cout << "Calling bp_tree destructor end" << std::endl;
}
